...
Это модель натренирована на наложение стиля картины "Звездная ночь":
...
Этот алгоритм использует уже обученную сверточную модель (например, VGG19). Он изменяет входное изображение, накладывая на него заданный стиль. 

Из-за того, что преобразование требует большое количество вычислений, рекомендуется использовать только на GPU.
